...
class ParameterTuner:
        ...
        try:
            ...
            for scenario in test_scenarios:
                ...
                for i, ref_point in enumerate(reference_trajectory):
                    ...
                    # 使用Pinocchio动力学模型更新状态
                    if control_cmd.joint_torques is not None:
                        # 方法1：使用力矩更新（通过Pinocchio正向动力学）
                        tau = control_cmd.joint_torques
                        
                        # 使用动力学引擎计算加速度
                        try:
                            # 使用Pinocchio的正向动力学：qdd = ABA(q, qd, tau)
                        ...
                        # 方法2：使用位置控制（如果只有位置输出）
                        # 使用PD控制器模拟执行器响应，然后通过Pinocchio计算动力学
                        target_pos = control_cmd.joint_positions
                        
                        # PD控制器参数（模拟执行器内部控制）
                        kp_actuator = 50.0  # 执行器比例增益
                        kd_actuator = 5.0   # 执行器微分增益
                        
                        # 计算执行器力矩
                        position_error = target_pos - q
                        velocity_error = -v  # 目标速度为0（位置控制）
                        
                        tau_actuator = kp_actuator * position_error + kd_actuator * velocity_error
                        tau_actuator = np.clip(tau_actuator, -100.0, 100.0)
                        
                        # 使用Pinocchio正向动力学计算加速度
                        try:
                            a = self.dynamics_engine.forward_dynamics(q, v, tau_actuator)
                            a = np.clip(a, -100.0, 100.0)
                        except Exception as e:
                            # 后备方案
                            mass = 1.0
                            damping = 0.3
                            a = (tau_actuator - damping * v) / mass
                            a = np.clip(a, -100.0, 100.0)
                        ...
                        # 计算能耗
                        power = np.sum(np.abs(tau_actuator * v))
                        energy_consumption += power * dt
                ...
                rms_tracking_error = np.sqrt(np.mean(np.array(tracking_errors)**2)) if tracking_errors else 1.0
                ...
                # 归一化能耗（避免数值过大）
                trajectory_duration = len(reference_trajectory) * dt
                normalized_energy = energy_consumption / max(trajectory_duration, 1.0)
                
                # 限制各项指标的范围，避免数值爆炸
                avg_tracking_error = np.clip(avg_tracking_error, 0, 10.0)
                rms_tracking_error = np.clip(rms_tracking_error, 0, 10.0)
                settling_time = np.clip(settling_time, 0, 10.0)
                overshoot = np.clip(overshoot, 0, 10.0)
                normalized_energy = np.clip(normalized_energy, 0, 1000.0)
                avg_vibration = np.clip(avg_vibration, 0, 100.0)
                
                # 加权性能分数（增强跟踪误差的权重，使用RMS而不是平均值）
                # 使用更大的缩放因子来增强参数敏感度
                scenario_score = (
                    self.performance_weights.tracking_accuracy * rms_tracking_error * 20.0 +  # 放大20倍
                    self.performance_weights.settling_time * settling_time * 2.0 +  # 放大2倍
                    self.performance_weights.overshoot * overshoot * 5.0 +  # 放大5倍
                    self.performance_weights.energy_efficiency * normalized_energy * 0.001 +
                    self.performance_weights.vibration_suppression * avg_vibration * 0.2  # 放大2倍
                )
    ...
    def _evaluate_trajectory_performance(
        ...
        try:
            ...
            # 计算平均分数
            final_score = total_score / max(path_count, 1)
            
            # 检查并处理NaN或Inf
            if np.isnan(final_score) or np.isinf(final_score):
                warnings.warn(f"轨迹性能评估产生无效值: {final_score}，返回惩罚值")
                return 1e6
    ...
    def _evaluate_vibration_performance(
        ...
        try:
            ...
            # 计算平均分数
            final_score = total_score / max(trajectory_count, 1)
            
            # 检查并处理NaN或Inf
            if np.isnan(final_score) or np.isinf(final_score):
                warnings.warn(f"抑振性能评估产生无效值: {final_score}，返回惩罚值")
                return 1e6
