    ...
    def _initialize_pools(self):
        ...
        if self.config.mode in [ParallelMode.PROCESS, ParallelMode.HYBRID]:
            try:
                ...
                # 某些受限环境（如缺少/禁用系统信号量）无法创建进程池，降级为线程模式
                print(f"警告: 无法创建进程池，已降级为线程池执行: {e}")
...
def parallelize(
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            # 仅处理第一个可并行化的序列参数（list/tuple，长度>1）
            first_list_arg = None
            first_list_arg_index = None
            for idx, arg in enumerate(args):
                if isinstance(arg, (list, tuple)) and len(arg) > 1:
                    first_list_arg = arg
                    first_list_arg_index = idx
                    break

            if first_list_arg is None or first_list_arg_index is None:
                # 没有可并行化的参数，直接执行
            ...
            with TaskScheduler(config) as scheduler:
                # 将列表参数分块并行执行：func 接收一个子列表并返回对应结果（通常也是 list）
                effective_chunk_size = max(1, int(chunk_size))
                chunks = [
                    list(first_list_arg[i:i + effective_chunk_size])
                    for i in range(0, len(first_list_arg), effective_chunk_size)
                ]

                def chunk_call(chunk):
                    new_args = list(args)
                    new_args[first_list_arg_index] = chunk
                    return func(*new_args, **kwargs)

                chunk_results = scheduler.submit_tasks(chunk_call, chunks)

                # 合并分块结果，保持顺序
                if all(isinstance(r, list) for r in chunk_results if r is not None):
                    merged: List[Any] = []
                ...
                if all(isinstance(r, np.ndarray) for r in chunk_results if r is not None):
                    arrays = [r for r in chunk_results if r is not None]
                    return np.concatenate(arrays, axis=0) if arrays else np.array([])

                # 无法合并时返回分块结果（或单元素映射结果）
