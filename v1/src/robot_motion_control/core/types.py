...
class RobotState:
    ...
    end_effector_transform: Matrix = field(default_factory=lambda: np.eye(4))  # 末端位姿 (4x4齐次变换矩阵)
...
class Waypoint:
    ...
    timestamp: Optional[float] = None  # 兼容字段：部分模块/测试使用 timestamp

    def __post_init__(self):
        # 兼容：允许通过 timestamp 传入时间约束
        if self.time_constraint is None and self.timestamp is not None:
            self.time_constraint = self.timestamp
