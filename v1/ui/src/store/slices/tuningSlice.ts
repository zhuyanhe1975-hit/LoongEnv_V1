...
interface TuningState {
  ...
  // Backend tuning integration
  backendStatus: 'idle' | 'running' | 'completed' | 'error' | 'stopped';
...
const tuningSlice = createSlice({
  ...
  reducers: {
    ...
    // Backend tuning state (used by TuningPage)
    setBackendTuningState: (
      state,
      action: PayloadAction<{ status: TuningState['backendStatus']; progress?: number; error?: string | null }>
    ) => {
      state.backendStatus = action.payload.status;
      if (typeof action.payload.progress === 'number') {
        state.backendProgress = action.payload.progress;
        state.autoTuningProgress = action.payload.progress;
      }
    ...
    setBackendTuningResults: (state, action: PayloadAction<any>) => {
      state.backendResults = action.payload;
      if (action.payload?.success) {
        state.backendStatus = 'completed';
        state.backendProgress = 100;
        state.autoTuningProgress = 100;
      } else {
